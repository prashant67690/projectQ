...
const Settings = () => {
  ...
  useEffect(() => {
    const fxn = async () => {
      ...
      if (isMentor) {
        res = await axios.get(
          `http://localhost:3000/api/mentor/${user.data.id}`,
          {
            withCredentials: true,
          },
        );
      } else {
        res = await axios.get(`http://localhost:3000/api/student/me`, {
          withCredentials: true,
        });
      }
    ...
    fxn();
  }, [setFirstName, setLastName, setEmail, setDept]);
  ...
  const formSubmitHandler = async (e) => {
    ...
    try {
      if (isMentor) {
        ...
        dispatch(setCredentialsMentor(payload));

        console.log(res);
      } else {
        const res = await axios.put(
          `http://localhost:3000/api/student/${user.data.id}`,
          data,
          {
            withCredentials: true,
          },
